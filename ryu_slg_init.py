...
class RyuSlgInit(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        self.logger.info('slg_id: {}'.format(slg_id))
        ...
        flow['actions'].append({
                        "type": "SET_FIELD",
                        "field": "eth_dst",
                        "value": slg[slg_id].interface[1].gateway.mac_address
                    })
        ...
        flow['match'] = {
                ...
                'ipv4_dst': slg[slg_id].interface[1].ip_address
            }
        ...
        flow['match'] = {
                ...
                'dl_type': 2054,
                'eth_dst': "ff:ff:ff:ff:ff:ff",
                'ipv4_dst': slg[slg_id].interface[2].ip_address
            }
        flow['actions'] = [{"type": "OUTPUT", "port": 4294967293}]
        print(flow)
        mod_flow_entry(datapath, flow, ofproto.OFPFC_ADD)
    ...
    def reply_arp(self, datapath, etherFrame, arpPacket, arp_dstIp, inPort):
        ...
        if datapath.id - 1 == 0:
            if arp_dstIp == "192.168.42.254":
                srcMac = "52:54:00:01:01:01"
                outPort = 1
            elif arp_dstIp == "172.16.1.1":
                srcMac = "52:54:00:00:00:01"
                outPort = 2
            else:
                self.logger.debug("unknown arp request received !")
        if datapath.id - 1 == 1:
            if arp_dstIp == "192.168.1.254":
                srcMac = "52:54:00:02:01:01"
                outPort = 1
            elif arp_dstIp == "172.16.2.1":
                srcMac = "52:54:00:00:00:02"
                outPort = 2
            else:
                self.logger.debug("unknown arp request received !")
        if datapath.id - 1 == 2:
            if arp_dstIp == "192.168.3.254":
                srcMac = "52:54:00:03:01:01"
                outPort = 1
            elif arp_dstIp == "172.16.3.1":
                srcMac = "52:54:00:00:00:03"
                outPort = 2
            else:
                self.logger.debug("unknown arp request received !")
